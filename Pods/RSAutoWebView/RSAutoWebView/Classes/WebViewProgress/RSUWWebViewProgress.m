...
#import "RSUWWebViewProgress.h"

static NSString * const RS_completeRPCURL = @"webviewprogressproxy:///complete";

const CGFloat RS_NJKInitialProgressValue = 0.1;
const CGFloat RS_NJKInteractiveProgressValue = 0.5;
const CGFloat RS_NJKFinalProgressValue = 0.9;

@implementation RSUWWebViewProgress {
...
- (void)startProgress {
    if (_progress < RS_NJKInitialProgressValue) {
        [self setProgress:RS_NJKInitialProgressValue];
    }
}

- (void)incrementProgress {
    CGFloat progress = self.progress;
    CGFloat maxProgress = _interactive ? RS_NJKFinalProgressValue : RS_NJKInteractiveProgressValue;
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    if ([request.URL.absoluteString isEqualToString:RS_completeRPCURL]) {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (interactive) {
        _interactive = YES;
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@'; document.body.appendChild(iframe);  }, false);", RS_completeRPCURL];
