    ...
    if (self.isCompletion) {
        if (self.error) {
            ...
            if (self.responseData.length > 0) {
                NSData *recvCopyData = [self.responseData copy];
...
- (NSArray *)cacheFileActions
{
    ...
    if (self.response && [self isCompletionWithOutError] && self.isDataUpdated) {
        ...
        response = [[NSHTTPURLResponse alloc] initWithURL:[NSURL URLWithString:self.url] statusCode:200 HTTPVersion:@"1.1" headerFields:respHeader];
    }
    
    NSMutableData *cacheData = [self.cacheFileData mutableCopy];
